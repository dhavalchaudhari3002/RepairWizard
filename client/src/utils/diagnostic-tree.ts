/**
 * Utility functions for working with diagnostic question trees
 */
...
/**
 * Traverses a diagnostic tree to find the next question based on user answers
 */
export function getNextQuestion(
  ...
  const currentQuestion = tree.questions[currentQuestionId];
  
  if (!currentQuestion) {
    return undefined;
  }
  
  // For single-choice questions, find the matching choice and its next question
  if (currentQuestion.type === 'single-choice' && answerIds.length === 1) {
    const selectedChoice = currentQuestion.choices?.find(choice => choice.id === answerIds[0]);
    
    if (selectedChoice?.nextQuestionId) {
      return tree.questions[selectedChoice.nextQuestionId];
    }
  }
  
  // For multi-choice or other question types, use the default next question
  if (currentQuestion.defaultNextQuestionId) {
    return tree.questions[currentQuestion.defaultNextQuestionId];
  }
  
  // If no next question is found, return undefined
  return undefined;
}
...
/**
 * Formats tree data from the database into the DiagnosticTree structure
 */
export function dbToTreeFormat(dbTree: any): DiagnosticTree {
  // If treeData is already a DiagnosticTree, return it
  if (typeof dbTree.treeData === 'object') {
    return dbTree.treeData as DiagnosticTree;
  }
  
  // Otherwise, try to parse it from JSON
  try {
    return JSON.parse(dbTree.treeData) as DiagnosticTree;
  } catch (error) {
    // If parsing fails, create a new basic tree
    return createBasicDiagnosticTree(
      dbTree.productCategory,
      dbTree.subCategory || undefined
    );
  }
}